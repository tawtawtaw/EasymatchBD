...
import {
  getAdminConsultantTariffs,
  updateAdminConsultantTariffs,
  type ConsultantTariffConfig,
} from "@/lib/admin";

type EditableTariff = ConsultantTariffConfig;

export function AdminConsultantTariffsPanel({
  onError,
  onMessage,
}: {
  ...
  const t = useTranslations("admin.consultantTariffs");
  ...
  const loadTariffs = useCallback(async () => {
    ...
    if (!token) return;
    const data = await getAdminConsultantTariffs(token);
    setTariffs(data);
    setDirty(false);
  }, []);
  ...
  function updateTariff(serviceType: string, patch: Partial<EditableTariff>) {
    setTariffs((current) =>
      current.map((row) =>
        row.serviceType === serviceType ? { ...row, ...patch } : row,
      ),
    );
    setDirty(true);
    onMessage(null);
  ...
  async function handleSave() {
    ...
    onError(null);
    try {
      const updated = await updateAdminConsultantTariffs(
        token,
        tariffs.map((row) => ({
          ...
          >
            <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
              <h3 className="text-base font-semibold text-zinc-900">
                {t(`services.${row.serviceType}` as "services.profile_assessment")}
              </h3>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="block text-sm">
                ...
                <input
                  ...
                  value={row.labelEn}
                  onChange={(event) =>
                    updateTariff(row.serviceType, { labelEn: event.target.value })
                  }
                />
              </label>
              <label className="block text-sm">
                ...
                <input
                  ...
                  value={row.labelBn ?? ""}
                  onChange={(event) =>
                    updateTariff(row.serviceType, { labelBn: event.target.value })
                  }
                />
              </label>
              <label className="block text-sm">
                ...
                <input
                  ...
                  value={row.priceBdt}
                  onChange={(event) =>
                    updateTariff(row.serviceType, { priceBdt: event.target.value })
                  }
                />
              </label>
              <label className="block text-sm">
              ...
              <label className="block text-sm">
                ...
                <input
                  ...
                  value={row.currency}
                  onChange={(event) =>
                    updateTariff(row.serviceType, { currency: event.target.value })
                  }
                />
              </label>
            </div>
